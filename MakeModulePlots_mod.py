...
def MakeModulePlot(arrays_of_data= [ [] ], row = [],col = [],nfig=5,hmin=-1,hmax=-1, percentile = 0.05, plotAverage = True, identifier="", xlabel="columns", data_label="", test_label="", israw = False, filename="", show_plot=True, save_plot=True):
    ...
    targetMaxValue = 1
    targetMinValue = -3
    if not (("BadBumpGaussian_refitted" in filename) or ("BadBumpVCal3_refitted" in filename)):
    ...
    maximum = hmax if hmax !=-1 else max(0,targetMaxValue)
    minimum = hmin if hmin !=-1 else max(0,targetMinValue)
    #print(hmax,hmin,maximum,minimum)
...
def MakeAllPlotsOneModule(inpath="./",mapsa="MaPSA",mapsaname="",modules=["chip1","chip2","chip3","chip4","chip5","chip6","chip7","chip8","chip9","chip10","chip11","chip12","chip13","chip14","chip15","chip16"], show_plot=True, save_plot=True):
    #print("MakeAllPlotsOneModule(",inpath,mapsa,mapsaname,",".join(modules),show_plot,save_plot,")")
    bases = ["BumpBonding_BadBumpGaussian_refitted","BumpBonding_BadBumpVCal3_refitted","BumpBondingv2_BadBumpGaussian_refitted","BumpBondingv2_BadBumpVCal3_refitted","BumpBonding_Noise_BadBump_refitted","BumpBondingv2_Noise_BadBump_refitted"]
    label = ["Bad Bump Test Gaussian Fit (through noise at BV = 0)","Bad Bump Test Re (through noise at BV = 0)","Bad Bump Test Gaussian Fit (through noise at BV = -10 V)","Bad Bump Test Re (through noise at BV = -10 V)","Bad Bump Noise Re (at BV = 0)","Bad Bump Noise Re (at BV = -10 V)"]
    zlabel = ["is bad bump","is bad bump","is bad bump","is bad bump","noise [CAL] (at BV = 0)","noise [CAL] (at BV = -10 V)"]
    isscurves = [False,False,False,False,False,False]
    THR   =     ["",   "",   "",   "",   "",   ""   ]
    doMeans =   [ True, True, True, True, True, True]
    Averaged =  [False,False,False,False,False,False]
    #print(mapsa)
    if mapsaname=="": mapsaname = mapsa
    for i in range(len(bases)):
        #if i > 1: continue
        ...
        if "v2_Noise_" in bases[i]: mymax = 8.
        if "BadBumpsThrougNoise" in bases[i]: mymax = 1.
        if "BadBumpGaussian_refitted" in bases[i] or "BadBumpVCal3_refitted" in bases[i]:
            mymax =  1
            mymin = -3
        outpath = inpath
        if outpath[-1] != '/': outpath = outpath + '/'
        Plot_Module(inpath=inpath,mapsa=mapsa,base=bases[i],modules=modules,isscurve=isscurves[i],s_type=THR[i],doMean=doMeans[i],data_label=zlabel[i],test_label=label[i],identifier=mapsaname,filename=outpath+mapsa+"_"+mybase,plotAverage=Averaged[i],hmax=mymax,hmin=mymin,show_plot=show_plot,save_plot=save_plot)
...
def PlotAllPlotsAllModuleAutomated(show_plot=True, save_plot=True):
    modulenames = ["AEMTec1","AEMTec2","AEMTec3","AEMTec4","AEMTec5","AEMTec6","AEMTec7","AEMTec8","AEMTec9","AEMTec10","HPK2_1","HPK15_1","HPK15_2","HPK16_1","HPK17_1","HPK18_1","HPK19_1","HPK25_2","HPK26_1","HPK26_2"]
    for m in modulenames:
        PlotAllPlotsOneModuleAutomated(m,show_plot=show_plot,save_plot=save_plot)
...
PlotAllPlotsAllModuleAutomated(show_plot=False,save_plot=True)
"""
