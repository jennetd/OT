...
from mpa_configurations import conf
import os.path
import glob
import re
        ...
        for r in row:
            for c in col:
                ...
                if r >=conf.nrowsnom or  c >= conf.ncolsnom:
                    ...
                    if r ==0 and arrays_of_data[n][pixelid] < 10 and test_label=='Pixel alive test':
                        print(n,c,arrays_of_data[n][pixelid])
...
def MakeAllPlotsOneModule(inpath="./",mapsa="MaPSA",mapsaname="",modules=["chip1","chip2","chip3","chip4","chip5","chip6","chip7","chip8","chip9","chip10","chip11","chip12","chip13","chip14","chip15","chip16"], show_plot=True, save_plot=True):
    ...
    bases = ["BumpBonding_BadBumpMap","BumpBonding_Noise_BadBump","mask_test","pixelalive"]
    label = ["Bad Bump Test (through noise at BV = -2V)","Identified abnormal pixel noise (at BV = -2V)","Pixel masking test","Pixel alive test"]
    zlabel = ["is bad bump","noise [CAL] (at BV = -2V)","is maskeable","recorded pulses"]
    isscurves = [False,False,False,False]
    THR   =     ["",   "",   "",   ""]
    doMeans =   [True, True, True, True]
    Averaged =  [False,False,False,False]
    #print(mapsa)
...
def PlotAllPlotsOneModuleAutomated(modulename, show_plot=True, save_plot=True):
    moduleid = "micross_test_"+modulename
    thepath = "../Results_MPATesting/Micross/Micross_"+modulename+"/"
